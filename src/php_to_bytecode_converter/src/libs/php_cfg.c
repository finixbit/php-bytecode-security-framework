...
char *
phpdbg_short_zval_print2 (zval *zv, int maxlen)
{
    char *decode = NULL;
    ...
    switch (Z_TYPE_P (zv))
        ...
        case IS_DOUBLE:
            ...
            break;
        case IS_STRING:
            spprintf (&decode, 0, "%s", Z_STRVAL_P (zv));
            break;
        case IS_RESOURCE:
        ...
        case IS_ARRAY:
            spprintf (&decode, 0, "array(%d)",
                      zend_hash_num_elements (Z_ARR_P (zv)));
            break;
        case IS_OBJECT:
            {
...
static void
zend_mark_reachable (zend_op *opcodes, zend_cfg *cfg,
                     zend_basic_block *b) /* {{{ */
{
            ...
            for (i = 0; i < b->successors_count; i++)
                {
                    ...
                    if (b->len != 0)
                        {
                            zend_uchar opcode
                                = opcodes[b->start + b->len - 1].opcode;
                            if (b->successors_count == 1)
                                ...
                                {
                                    ZEND_ASSERT (opcode == ZEND_SWITCH_LONG
                                                 || opcode
                                                        == ZEND_SWITCH_STRING);
...
static void
zend_mark_reachable_blocks (const zend_op_array *op_array, zend_cfg *cfg,
                            int start) /* {{{ */
{
    zend_basic_block *blocks = cfg->blocks;
                    ...
                    for (j = 0; j < op_array->last_live_range; j++)
                        {
                            ...
                            if (b->flags & ZEND_BB_REACHABLE)
                                {
                                    while (b->len > 0
                                           && op_array->opcodes[b->start].opcode
                                                  == ZEND_NOP)
                                        {
                                            /* check if NOP breaks incorrect
                                             * smart branch */
                                            if (b->len == 2
                                                && (op_array->opcodes[b->start
                                                                      + 1]
                                                            .opcode
                                                        == ZEND_JMPZ
                                                    || op_array->opcodes
                                    ...
                                    if (b->len == 0
                                        && (uint32_t)b->successors[0]
                                               == block_map[live_range->end])
                                        {
                                            /* mark as removed (empty live
                                             * range) */
                                    ...
                                    if (!(b->flags
                                        ...
                                        {
                                            if ((cfg->flags
                                                 & ZEND_CFG_SPLIT_AT_LIVE_RANGES))
                                                {
                                                    changed = 1;
                                                    zend_mark_reachable (
                                                        op_array->opcodes, cfg,
                                                        b);
                                                }
                                            else
                                                {
                                ...
                                {
                                    ZEND_ASSERT (!(
                                        blocks[block_map[live_range->end]].flags
                                        & ZEND_BB_REACHABLE));
                                }
                        }
                    ...
                    for (j = 0; j < op_array->last_try_catch; j++)
                        {
                            ...
                            b = blocks
                                + block_map[op_array->try_catch_array[j]
                                                .try_op];
                            if (!(b->flags & ZEND_BB_REACHABLE))
                                {
...
uint32_t
zend_optimizer_classify_function (zend_string *name, uint32_t num_args)
{
...
static void
initialize_block (zend_basic_block *block)
{
    block->flags = 0;
...
int
zend_build_cfg (zend_arena **arena, const zend_op_array *op_array,
                uint32_t build_flags, zend_cfg *cfg) /* {{{ */
{
    ...
    cfg->flags = build_flags
                 & (ZEND_CFG_SPLIT_AT_LIVE_RANGES | ZEND_CFG_STACKLESS
                    | ZEND_CFG_RECV_ENTRY);

    cfg->map = block_map
        = zend_arena_calloc (arena, op_array->last, sizeof (uint32_t));
            ...
            switch (opline->opcode)
                ...
                case ZEND_INIT_NS_FCALL_BY_NAME:
                    ...
                    if (opline->opcode == ZEND_INIT_NS_FCALL_BY_NAME)
                        {
                            /* The third literal is the lowercased unqualified
                             * name */
                            zv += 2;
                        }
                    // if ((fn = zend_hash_find_ptr(EG(function_table),
                    // Z_STR_P(zv))) != NULL) { 	if (fn->type ==
                    // ZEND_INTERNAL_FUNCTION) { 		flags |=
                    // zend_optimizer_classify_function( 			Z_STR_P(zv),
                    // opline->extended_value);
                    // 	}
                    // }
                    break;
                case ZEND_FAST_CALL:
                    BB_START (OP_JMP_ADDR (opline, opline->op1)
                              - op_array->opcodes);
                    BB_START (i + 1);
                    break;
                case ZEND_FAST_RET:
                ...
                case ZEND_JMP:
                    ...
                    break;
                case ZEND_JMPZNZ:
                    BB_START (OP_JMP_ADDR (opline, opline->op2)
                              - op_array->opcodes);
                    BB_START (ZEND_OFFSET_TO_OPLINE_NUM (
                        op_array, opline, opline->extended_value));
                    if (i + 1 < op_array->last)
                        {
                            BB_START (i + 1);
                        }
                ...
                case ZEND_SWITCH_STRING:
                    {
                        // printf("ZEND_SWITCH_STRING, line_%d type_%d\n",
                        // opline->lineno, opline->op2_type); if
                        // (opline->op2_type == IS_CONST) {
                        // 	printf("IS_CONST\n");
                        // 	zval *literal = RT_CONSTANT(opline,
                        // opline->op2); 	printf("M1\n"); 	char *decode =
                        // phpdbg_short_zval_print2(literal, 20);
                        ...
                        // 	printf("ZEND_RT_CONSTANTS\n");
                        // }

                        // if(Z_ARRVAL_P(CRT_CONSTANT(opline->op2))) {
                        // 	printf("1212\n");
                        // }

                        // HashTable *jumptable =
                        // Z_ARRVAL_P(CRT_CONSTANT(opline->op2));
                        HashTable *jumptable
                            = Z_ARRVAL_P (RT_CONSTANT (opline, opline->op2));
                        zval *zv;

                        ZEND_HASH_FOREACH_VAL (jumptable, zv)
                        {
                            BB_START (ZEND_OFFSET_TO_OPLINE_NUM (
                                op_array, opline, Z_LVAL_P (zv)));
                        }
                    ...
                    if (opline->extended_value & ZEND_FETCH_LOCAL)
                        {
                            flags |= ZEND_FUNC_INDIRECT_VAR_ACCESS;
                              ...
                              & (ZEND_FETCH_GLOBAL | ZEND_FETCH_GLOBAL_LOCK))
    ...
    if ((build_flags & ZEND_CFG_NO_ENTRY_PREDECESSORS) && op_array->last > 0
        && block_map[0] > 1)
        {
            extra_entry_block = 1;
        }
    ...
    cfg->blocks = blocks
        = zend_arena_calloc (arena, sizeof (zend_basic_block), blocks_count);
            ...
            if (block_map[i])
                {
                    if (blocks_count >= 0)
                        {
                            blocks[blocks_count].len
                                = i - blocks[blocks_count].start;
                        }
                    blocks_count++;
    ...
    for (j = 0; j < blocks_count; j++)
        {
            ...
            switch (opline->opcode)
                ...
                case ZEND_JMP:
                    block->successors_count = 1;
                    block->successors[0]
                        = block_map[OP_JMP_ADDR (opline, opline->op1)
                                    - op_array->opcodes];
                    break;
                case ZEND_JMPZNZ:
                    block->successors_count = 2;
                    block->successors[0]
                        = block_map[OP_JMP_ADDR (opline, opline->op2)
                                    - op_array->opcodes];
                    block->successors[1] = block_map[ZEND_OFFSET_TO_OPLINE_NUM (
                        op_array, opline, opline->extended_value)];
                    break;
                case ZEND_JMPZ:
                case ZEND_JMPNZ:
                ...
                case ZEND_FE_FETCH_RW:
                    block->successors_count = 2;
                    block->successors[0] = block_map[ZEND_OFFSET_TO_OPLINE_NUM (
                        op_array, opline, opline->extended_value)];
                    block->successors[1] = j + 1;
                    break;
                case ZEND_FE_RESET_R:
                ...
                case ZEND_SWITCH_STRING:
                    {
                        ...
                        block->successors_count
                            = 2 + zend_hash_num_elements (jumptable);
                        block->successors = zend_arena_calloc (
                            arena, block->successors_count, sizeof (int));

                        ZEND_HASH_FOREACH_VAL (jumptable, zv)
                        {
                            block->successors[s++]
                                = block_map[ZEND_OFFSET_TO_OPLINE_NUM (
                                    op_array, opline, Z_LVAL_P (zv))];
                        }
                        ZEND_HASH_FOREACH_END ();
...
int
zend_cfg_build_predecessors (zend_arena **arena, zend_cfg *cfg) /* {{{ */
{
    int j, s, edges;
    zend_basic_block *b;
    zend_basic_block *blocks = cfg->blocks;
    ...
    cfg->predecessors = predecessors
        = (int *)zend_arena_calloc (arena, sizeof (int), edges);
                    ...
                    for (s = 0; s < blocks[j].successors_count; s++)
                        {
                            ...
                            for (p = 0; p < s; p++)
                                {
                                    if (blocks[j].successors[p]
                                        == blocks[j].successors[s])
                                        {
                                            duplicate = 1;
                                            break;
                                        }
                            ...
                            if (!duplicate)
                                {
                                    zend_basic_block *b
                                        = blocks + blocks[j].successors[s];

                                    predecessors[b->predecessor_offset
...
static void
compute_postnum_recursive (int *postnum, int *cur, const zend_cfg *cfg,
                           int block_num) /* {{{ */
{
    int s;
...
/* Computes dominator tree using algorithm from "A Simple, Fast Dominance
 * Algorithm" by Cooper, Harvey and Kennedy. */
int
zend_cfg_compute_dominators_tree (const zend_op_array *op_array,
                                  zend_cfg *cfg) /* {{{ */
{
    zend_basic_block *blocks = cfg->blocks;
            ...
            for (j = 1; j < blocks_count; j++)
                {
                    ...
                    for (k = 0; k < blocks[j].predecessors_count; k++)
                        {
                            int pred
                                = cfg->predecessors[blocks[j].predecessor_offset
                                                    + k];
...
static int
compare_block_level (const block_info *a, const block_info *b)
{
    return b->level - a->level;
}
static void
swap_blocks (block_info *a, block_info *b)
{
    block_info tmp = *a;
    *a = *b;
    *b = tmp;
...
static int
dominates (zend_basic_block *blocks, int a, int b) /* {{{ */
{
...
int
zend_cfg_identify_loops (const zend_op_array *op_array, zend_cfg *cfg) /* {{{ */
{
    int i, j, k, n;
    int time;
    zend_basic_block *blocks = cfg->blocks;
    ...
    /* We don't materialize the DJ spanning tree explicitly, as we are only
     * interested in ancestor queries. These are implemented by checking
     * entry/exit times of the DFS search. */
    entry_times
        = do_alloca (2 * sizeof (int) * cfg->blocks_count, tree_use_heap);
    ...
    /* Sort blocks by decreasing level, which is the order in which we want to
     * process them */
    sorted_blocks = do_alloca (sizeof (block_info) * cfg->blocks_count,
                               sorted_blocks_use_heap);
    ...
    for (n = 0; n < cfg->blocks_count; n++)
        {
            ...
            zend_bitset_clear (work.visited,
                               zend_bitset_len (cfg->blocks_count));
            for (j = 0; j < blocks[i].predecessors_count; j++)
                {
                    int pred
                        = cfg->predecessors[blocks[i].predecessor_offset + j];
                    ...
                    /* In a loop back-edge (back-join edge), the successor
                       dominates the predecessor.  */
                    if (dominates (blocks, i, pred))
                        ...
                        {
                            /* Otherwise it's a cross-join edge.  See if it's a
                               branch to an ancestor on the DJ spanning tree. */
                            if (entry_times[pred] > entry_times[i]
                                && exit_times[pred] < exit_times[i])
                                {

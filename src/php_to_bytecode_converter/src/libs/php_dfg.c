...
int
zend_build_dfg (const zend_op_array *op_array, const zend_cfg *cfg,
                zend_dfg *dfg, uint32_t build_flags) /* {{{ */
{
    int set_size;
                            ...
                            if (next < end && next->opcode == ZEND_OP_DATA)
                                {
                                    if (next->op1_type
                                        ...
                                        {
                                            var_num
                                                = EX_VAR_TO_NUM (next->op1.var);
                                            if (!DFG_ISSET (def, set_size, j,
                                                            var_num))
                                                {
                                                    DFG_SET (use, set_size, j,
                                                             var_num);
                                                }
                                        }
                                    if (next->op2_type
                            ...
                            if (opline->op1_type == IS_CV)
                                {
                                    ...
                                    switch (opline->opcode)
                                        ...
                                        op1_def:
                                            /* `def` always come along with dtor
                                             * or separation, thus the origin
                                             * var info might be also `use`d in
                                             * the feature(CG) */
    ...
    /* Calculate "in" and "out" sets */
    {
        ...
        while (!zend_bitset_empty (worklist, worklist_len))
            {
                /* We use the last block on the worklist, because predecessors
                 * tend to be located before the succeeding block, so this
                 * converges faster. */
                ...
                if (blocks[j].successors_count != 0)
                    {
                        zend_bitset_copy (
                            DFG_BITSET (out, set_size, j),
                            DFG_BITSET (in, set_size, blocks[j].successors[0]),
                            set_size);
                        for (k = 1; k < blocks[j].successors_count; k++)
                            {
                                zend_bitset_union (
                                    ...
                                    DFG_BITSET (in, set_size,
                                                blocks[j].successors[k]),
                                    set_size);
                            }
                    }
                else
                    {
                        zend_bitset_clear (DFG_BITSET (out, set_size, j),
                                           set_size);
                    }
                zend_bitset_union_with_difference (
                    tmp, DFG_BITSET (use, set_size, j),
                    DFG_BITSET (out, set_size, j),
                    DFG_BITSET (def, set_size, j), set_size);
                if (!zend_bitset_equal (DFG_BITSET (in, set_size, j), tmp,
                                        set_size))
                    {
                        zend_bitset_copy (DFG_BITSET (in, set_size, j), tmp,
                                          set_size);

                        /* Add predecessors of changed block to worklist */
                        {
                            int *predecessors
                                = &cfg->predecessors[blocks[j]
                                                         .predecessor_offset];
                            for (k = 0; k < blocks[j].predecessors_count; k++)
                                {
                                    zend_bitset_incl (worklist,
                                                      predecessors[k]);
                                }
                        }
                    }
